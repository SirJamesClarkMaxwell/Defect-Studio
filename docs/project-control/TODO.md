...
- Keep `scripts/Tooling.bat setup` as the canonical local setup entrypoint
- Track only significant tasks/features in this TODO; do not add tiny fixes

## Current priority queue (ordered)
- [ ] P1: Start T13 bug-fix pass from the newly added product issues
...
### [x] T10 - Editing workflow polish (`task/10-editing-workflow-polish`)
  ...
  - [x] important bug: exporting things like `project_appearance` was creating nested `config->project->config->project`
  - [x] add external project folder
  - [x] add project workflow: open, create, recent open
  ...
  - [x] keep Scene Outliner trees expanded when actions are triggered from context menus
...
### [x] T12 - Migrate Windows build scripts to Python (`task/12-scripts-python-migration`)
  - [x] Add uv as a project tooling dependency
  - [x] Create Python tooling project files and sync `.venv` during setup
  - [x] Add `scripts/setup.py` and `scripts/verify_build.py` as the underlying Python workflows
  - [x] Add a single Windows bootstrap wrapper: `scripts/Tooling.bat` / `scripts/Tooling.ps1`
  - [x] Make the bootstrap wrapper install `uv` when needed and install Python 3.11+ through uv-managed Python before dispatching tooling
  - [x] Keep the setup/build/run scope Windows-only for now and defer WSL/Linux parity to `T12a`
  - [x] Run the full setup wrapper end-to-end without skips: uv, `.venv`, submodules, premake, MSVC detection, build, and app launch
  - [x] Keep `scripts/Run.bat` / `scripts/Run.ps1` as the separate application launcher
  - [x] Keep `scripts/Setup.*` and `scripts/Verify-Build.*` as compatibility aliases
...
## Current focus
- Recently completed: **T12 Windows tooling migration (Python setup/verify workflows, single Windows bootstrap wrapper, separate app runner)**
- Active branch: **task/12-scripts-python-migration**
- Next deferred follow-up: **T12a - Cross-platform tooling follow-up**
